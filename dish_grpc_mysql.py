#!/usr/bin/python3
"""Write Starlink user terminal data to a mysql/maria database.

This script pulls the current status info and/or metrics computed from the
history data and writes them to the specified mysql database either once or
in a periodic loop.
...
def parse_args():
    parser = dish_common.create_arg_parser(output_description="write it to a mysql database")

    parser.add_argument("database", help="Database URL to use")

    group = parser.add_argument_group(title="Mysql database options")
